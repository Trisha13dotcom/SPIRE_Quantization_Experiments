...
class LanguageBrain(sb.core.Brain):
    ...
    def compute_objectives(self, predictions, batch, stage):
     """Computes the loss using language-id as label."""
     ...
     if stage != sb.Stage.TRAIN:
        ...
        self.all_preds.extend(
            pred_labels.detach().cpu().numpy().tolist()
        )

        self.all_targets.extend(
            langid.detach().cpu().numpy().tolist()
        )
...
def dataio_prep_shards(hparams):
    ...
    def audio_pipeline(sample_dict: Dict, random_chunk=True):
        ...
        audio_tensor = torch.load(io.BytesIO(sample_dict["audio.pth"]))
        # audio_tensor = sample_dict["audio.pth"]
        # print(sample_dict.keys())
        
        # print(type(sample_dict["audio.pth"]))
        # print(type(sample_dict["language_id"]))
        # print(sample_dict["language_id"])
        # print(sample_dict["audio.pth"][:20])
